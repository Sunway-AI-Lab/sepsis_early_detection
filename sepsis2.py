...
FEATURE_IMPORTANCE_PATH = os.path.join(SAVE_DIR, 'feature_importance.joblib')  # New: Save feature importance
...
THRESHOLD = 0.15  # Lowered from 0.3 to match test.py
...
if 'Lactate' in X.columns and 'WBC' in X.columns:
    X['Lactate_WBC'] = X['Lactate'] / (X['WBC'] + 1e-6)  # Avoid division by zero
if 'HR' in X.columns and 'MAP' in X.columns:
    X['HR_MAP'] = X['HR'] / (X['MAP'] + 1e-6)

# Clinically relevant features
priority_features = ['HR', 'Temp', 'Resp', 'WBC', 'Lactate', 'MAP', 'Creatinine', 'BUN', 'Glucose', 'Potassium', 'Age', 'ICULOS', 'Lactate_WBC', 'HR_MAP']
...
joblib.dump(feature_importance, FEATURE_IMPORTANCE_PATH)  # Save feature importance
...
if USE_SMOTE:
    print("\nApplying SMOTE...")
    smote = SMOTE(random_state=RANDOM_STATE, sampling_strategy=0.8)  # Increased from 0.5
...
def focal_loss(gamma=3.0, alpha=0.75):  # Increased gamma and alpha
...
for fold, (train_idx, val_idx) in enumerate(skf.split(X_train_scaled, y_train)):
    ...
    early_stopping = tf.keras.callbacks.EarlyStopping(
        monitor='val_auc', patience=20, mode='max', restore_best_weights=True, verbose=1  # Increased patience
    )
...
model = create_model(n_features)
early_stopping = tf.keras.callbacks.EarlyStopping(
    monitor='val_auc', patience=20, mode='max', restore_best_weights=True, verbose=1
)
...
best_model = tf.keras.models.load_model(MODEL_SAVE_PATH, custom_objects={'focal_loss_fn': focal_loss(gamma=3.0, alpha=0.75)})
